...
8. Make sure `nm_d8.js` and the included QuickJS script `read_d8_stdin.js` (see [Notes](https://github.com/guest271314/native-messaging-d8#notes)) is executable. To download V8 using [`jsvu`](https://github.com/GoogleChromeLabs/jsvu)
...
[QuickJS Javascript Engine](https://bellard.org/quickjs/quickjs.html) `std.open()` to open the file descriptor `/proc/<PID>/fd/0` that we send from `d8` as argument to QuickJS script `read_d8_stdin.js`, read STDIN to `d8` with `std.read()`, then echo the STDIN to the current `d8` process to `d8`. 
